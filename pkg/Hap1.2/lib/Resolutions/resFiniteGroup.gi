...
if tietze then
   if (v = 0)  then Append(Spheres,[l]);fi;	
...
for g in [1..N] do
if MC[j][g]=0 then Append(temp,[ [j,g]]);
fi;
od;
od;
...
if i<1 then return [[-1,1]]; else	#in the function were arrived at by trial and
   ...
   z:=[AbsoluteValue(x[1]),x[2]];
      ...
      if i<K then ComputedContractions[i][z[1]][z[2]]:=ChangeSign(x[1],c); fi; 	
      return c;
      fi;
   else
...
Append(PseudoBoundary[i],[Diff]);
Append(MaxComplex[i+1],[ShallowCopy(l)]);
MC[p[1]][p[2]]:=1;
