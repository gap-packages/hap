...
if Length(arg)>1 then 
if IsInt(arg[2]) then scl:=arg[2];  else char:=arg[2]; fi; 
fi;

if Length(arg)>2 then 
if IsInt(arg[3]) then scl:=arg[3];  else char:=arg[3]; fi;
fi;
...
if f{[1,2,3,4]}="ATOM" then  
if  f{[22]}=char then
v:=[f{[23,24,25,26]},f{[32,33,34,36,37,38]}, f{[40,41,42,44,45,46]}, f{[48,49,50,52,53,54]}]; 
Apply(v,EvalString);
v[2]:=(scl*v[2]/1000);
v[3]:=(scl*v[3]/1000);
v[4]:=(scl*v[4]/1000);
...
b[2]:=b[2]/avg;
b[3]:=b[3]/avg;
Add(B,StructuralCopy(b));  fi;
cnt:=cnt+1;; avg:=1; b:=v{[2,3,4]}; 
else b:=b+v{[2,3,4]}; avg:=avg+1;
fi;
od;

A:=StructuralCopy(B);

Print("Read chain of ",Length(A)," amino acids containing ",atoms," atoms.\n");
