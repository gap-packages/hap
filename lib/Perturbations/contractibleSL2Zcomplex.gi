...
#############
EdgeFinder1:=function(g)
local
	S,T,
	m,index;
#index:=SimplifyGeneratorsRepresentation(GeneratorsRepresentation(g));
index:=GeneratorsRepresentation(g);
S:=[[0,-1],[1,0]];
T:=[[1,1],[0,1]];
m:=Length(index);
if index[m]<0 then return g;fi;
if index[m]=0 then return g*S;fi;
if index[m]>0 then return g*T^-1;fi;
end;
#############
...
return EdgeFinder(SimplifyGeneratorsRepresentation(GeneratorsRepresentation(g)));
end;
##################
...
#d:=GeneratorsRepresentation(g);
#edge:=RefineEdge(EdgeFinder(d));
edge:=EdgeFinder(d);
#edge:=CanonicalRightCosetElement(St1,EdgeFinder(d)^-1)^-1;
return AddList(Chomotopy(edge*S*edge^-1*g),edge);
end;
#################
pos:=function(g)
...
for i in [1..Length(path)] do
	#Add(g,[Action(1,1,pos(path[i])),pos(CanonicalRightCosetElement(St1,path[i]^-1)^-1)]);
