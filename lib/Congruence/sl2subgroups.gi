...
v:=1;;
nodes:=[];
if not S in G then v:=v+1; AddDictionary(leaves,S,v); Add(nodes,S);fi;
if not U1 in G then v:=v+1; AddDictionary(leaves,U1,v); Add(nodes,U1);fi;
if not U2 in G then v:=v+1; AddDictionary(leaves,U2,v); Add(nodes,U2);fi;
