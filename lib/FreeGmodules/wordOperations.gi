...
function(arg)
local w,p,v,pos,i,ww,s;

w:=arg[1];
if Length(arg)=2 then p:=arg[2]; else p:=0; fi;

if p=0  then    #Changed this, July 2026
#######################################
...
while i <=Length(v)-1 do
  if v[i][1][1]=-v[i+1][1][1] and v[i][1]{[2..Length(v[i][1])]}=v[i+1][1]{[2..Length(v[i+1][1])]} then 
     s:=v[i][2]-v[i+1][2];
     v[i][1][1]:=SignInt(s)*v[i][1][1];
  Append(ww,MultiplyWord(AbsInt(s), [ v[i][1] ]) );
     i:=i+2;
  else
  Append(ww,MultiplyWord(v[i][2],[v[i][1]]));
     i:=i+1;
  fi;
od;
if i=Length(v) then
